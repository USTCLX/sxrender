...
;(function(factory){
    if(typeof define==='function'&&define.amd){
        ...
        define("SXRender",[],factory);
    }else if(typeof module ==='object' &&module.exports){
        // Node/CommonJS
        module.exports = factory()
    }else{
        // Browser globals
        window.SXRender = factory();
    }
}(function(){
    var nextID = 1;       //物体ID
    ...
    var Utils = {
         ...
         */
        checkClickElm: function(objs,clickPos,contentOffset){
            var obj = null;
            var pos = clickPos;
         ...
         * @return {fun}                 p关于t的函数
         */
        calTimingFunctionBySpring:function(damping,stiffness,initialVelocity,startX) {
            var c = damping;
            var k = stiffness;
     ...
     * 画板构造函数
     * @param  {object} opts 
     * @id-canvas原素实例，w-canvas宽度，h-canvas高度，contentW-画板内容宽度，contextH-画板内容高度
     * @return {object}      画板实例
     */
    var SXRender = function (opts){
...
// console.log('cur v',this._contentVelcoity.y);
...
// tempSpringTimeFun = Utils.calTimingFunctionBySpring(12,180,this._contentVelcoity.y,100);
// p = tempSpringTimeFun(timeStep/1000)

// factor = startPos + p*(stopPos-startPos);

// // this.springOffset.y = factor;


// clearInterval(this._animation);          //清除正在进行的其他动画
// this._animation = setInterval(function(){
//     if(timeTick<=1000){
//         p = tempSpringTimeFun(timeTick/1000);
//         timeTick += timeStep;

//         this.springOffset.y = startPos + p*(stopPos-startPos);
//         // this.reRender();
//     }else{
//         this.springOffset.y = 0;
//         this.reRender();
